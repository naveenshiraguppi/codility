...
select e1.event_type eventtype, e3.value-e1.value
from events e1
inner join (
        select e.event_type, MAX(e.time) as maxtime
        from events e
        where time != (select MAX(time) from events group by event_type having event_type=e.event_type)
        group by event_type
) e2
on e1.event_type = e2.event_type and e1.time=e2.maxtime
inner join (
        select e.event_type, e.value
        from events e
        where time = (select MAX(time) from events group by event_type having event_type=e.event_type)
) e3
on e1.event_type = e3.event_type and e1.time=e2.maxtime
