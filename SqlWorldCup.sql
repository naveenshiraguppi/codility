...
SELECT t.team_id, t.team_name, SUM(host.hostwins+host.hostdraws+guest.guestwins+ guest.guestdraws) as points
from teams t
inner join (
select teams.team_id, teams.team_name, 
CASE 
    WHEN matches.guest_goals>matches.host_goals THEN 3 ELSE 0
END as guestwins,
CASE 
    WHEN matches.guest_goals=matches.host_goals THEN 1 ELSE 0
END as guestdraws,
CASE 
    WHEN matches.host_goals>matches.guest_goals THEN 3 ELSE 0
END as hostwins,
CASE 
    WHEN matches.host_goals=matches.guest_goals THEN 1 ELSE 0
END as hostdraws
from teams
left join matches
...
from teams
left join matches
on matches.guest_team = teams.team_id
) guest
